...
class OrderDetail(DetailView):
	...
	def dispatch(self, request, *args, **kwargs):
		try:
			...
			user_checkout = UserCheckout.objects.raw('SELECT * from orders_usercheckout where id = %s ',[user_check_id])
			user_checkout=list(user_checkout)
			# print "YES YES"
			user_checkout=user_checkout[0]
		except UserCheckout.DoesNotExist:
			# user_checkout = UserCheckout.objects.get(user=request.user)
			user_checkout = UserCheckout.objects.raw('SELECT * from orders_usercheckout where user = %s ',[request.user])
			user_checkout=list(user_checkout)
			# print "YES NO"
			user_checkout=user_checkout[0]
		except:
...
class OrderList(LoginRequiredMixin, ListView):
	...
	queryset = Order.objects.raw('SELECT * from orders_order ')
